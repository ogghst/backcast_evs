...
async def test_get_users_admin_only(async_client: AsyncClient, db_session: Any) -> None:
    ...
    admin, admin_headers = await create_user_and_token(
        db_session, "admin@test.com", "admin"
    )
    user, user_headers = await create_user_and_token(
        db_session, "user@test.com", "viewer"
    )
...
async def test_update_user_self(async_client: AsyncClient, db_session: Any) -> None:
    user, headers = await create_user_and_token(
        db_session, "update_self@test.com", "viewer"
    )

    payload = {"full_name": "New Name Self"}

    resp = await async_client.put(
        f"{settings.API_V1_STR}/users/{user.id}", json=payload, headers=headers
    )
    assert resp.status_code == 200
    data = resp.json()
...
async def test_delete_user_admin_only(
    ...
    resp = await async_client.delete(
        f"{settings.API_V1_STR}/users/{target.id}", headers=admin_headers
    )
...
async def test_create_user_admin_only(
    ...
    admin, admin_headers = await create_user_and_token(
        db_session, "admin_cr@test.com", "admin"
    )
    user, user_headers = await create_user_and_token(
        db_session, "user_cr@test.com", "viewer"
    )
    ...
    resp = await async_client.post(
        f"{settings.API_V1_STR}/users", json=payload, headers=user_headers
    )
    assert resp.status_code == 403
...
@pytest.mark.asyncio
async def test_read_user_permissions(
    async_client: AsyncClient, db_session: Any
) -> None:
    admin, admin_headers = await create_user_and_token(
        db_session, "admin_read@test.com", "admin"
    )
    user1, user1_headers = await create_user_and_token(
        db_session, "read1@test.com", "viewer"
    )
    user2, user2_headers = await create_user_and_token(
        db_session, "read2@test.com", "viewer"
    )

    # 1. Admin reads user1 -> 200
    resp = await async_client.get(
        f"{settings.API_V1_STR}/users/{user1.id}", headers=admin_headers
    )
    assert resp.status_code == 200

    # 2. User1 reads self -> 200
    resp = await async_client.get(
        f"{settings.API_V1_STR}/users/{user1.id}", headers=user1_headers
    )
    assert resp.status_code == 200

    # 3. User1 reads user2 -> 403
    resp = await async_client.get(
        f"{settings.API_V1_STR}/users/{user2.id}", headers=user1_headers
    )
    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_read_non_existent_user(
    async_client: AsyncClient, db_session: Any
) -> None:
    admin, admin_headers = await create_user_and_token(
        db_session, "admin_404@test.com", "admin"
    )
    fake_id = uuid4()

    resp = await async_client.get(
        f"{settings.API_V1_STR}/users/{fake_id}", headers=admin_headers
    )
...
async def test_create_user_validation(
    ...
    admin, admin_headers = await create_user_and_token(
        db_session, "admin_val@test.com", "admin"
    )
    ...
    resp = await async_client.post(
        f"{settings.API_V1_STR}/users", json=payload_short_pw, headers=admin_headers
    )
    ...
    resp = await async_client.post(
        f"{settings.API_V1_STR}/users", json=payload_bad_email, headers=admin_headers
    )
    assert resp.status_code == 422
...
async def test_update_user_permissions(
    ...
    user1, user1_headers = await create_user_and_token(
        db_session, "upd1@test.com", "viewer"
    )
    ...
    resp = await async_client.put(
        f"{settings.API_V1_STR}/users/{user1.id}", json=payload, headers=admin_headers
    )
    assert resp.status_code == 200

    # 2. User1 updates user2 -> 403
    resp = await async_client.put(
        f"{settings.API_V1_STR}/users/{user2.id}", json=payload, headers=user1_headers
    )
    assert resp.status_code == 403
...
async def test_delete_user_permissions(
    ...
    user1, user1_headers = await create_user_and_token(
        db_session, "del1@test.com", "viewer"
    )
