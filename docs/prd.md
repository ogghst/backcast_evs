...
#### 8.4.1 Entity Versioning (Composite Identity)
...
The system shall extend versioning capabilities for WBE and CostElement entities through branch support. Branch-enabled entities must maintain an additional `branch` field that identifies which branch the entity version belongs to.

The system shall support two types of branches:

- **Main Branch**: The production branch (named "main") containing all approved, active project data. All operational reporting and EVM calculations default to the main branch unless a specific branch is selected.
- **Change Order Branches**: Isolated branches (named `co-{identifier}`) automatically created for each change order, enabling independent modifications without affecting main branch data until merged.

#### 8.4.3 Branch Creation and Management

When a change order is created, the system shall automatically create a corresponding branch with a unique identifier derived from the change order ID (format: `co-{short_id}`).

The system shall support the following branch operations:

- **Branch Creation (Deep Copy)**: When a branch is created, the system shall perform a **Deep Copy** of the Project and all its WBEs from the 'main' branch to the new branch. This ensures complete isolation.
- **Branch Listing**: Ability to list all branches associated with a project.
- **Branch Deletion**: Deleting a branch (hard or soft) is a contained operation that removes the specific partitioned data for that branch.

#### 8.4.4 Branch Context and Filtering

The system shall utilize the Composite Primary Key `(id, branch)` to natively enforce branch context.
- All database queries for branch-enabled entities must include the `branch` parameter in their primary lookup or filter.
- **Strict Isolation**: There is no "fallback" to the main branch. If an entity is not found in the current branch, it does not exist in that context.
- **Context Persistence**: The branch selection is request-scoped and persisted per user session.
...
The system shall provide comprehensive branch comparison capabilities to analyze differences between any branch and the main branch (or between any two branches). The comparison must identify:

- **Created Entities**: New WBEs or CostElements added in the branch
- **Updated Entities**: Entities modified in the branch, showing field-level differences
- **Deleted Entities**: Entities soft-deleted in the branch
- **Financial Impact**: Summary of budget, revenue, and forecast changes between branches
- **Change Summary**: Aggregated statistics showing the scope and magnitude of branch modifications

Branch comparison results must be available through both API endpoints and user interface views, enabling users to review changes before merging.

#### 8.4.7 Branch Merging

The system shall support merging change order branches into the main branch upon change order approval and implementation. The merge operation must:

- Create new versions of all modified entities in the main branch, preserving the previous main branch versions
- Apply all creates, updates, and deletes from the branch to the main branch
- Handle entity relationships correctly, ensuring WBE-CostElement hierarchies are maintained
- Mark all branch entities with status="merged" after successful merge
- Generate notifications to relevant stakeholders upon merge completion
- Validate merge operations to prevent conflicts and ensure data integrity

The merge process must be atomic: either all changes are applied successfully, or the operation fails and no changes are made to the main branch. The system shall provide merge preview capabilities allowing users to review what will be merged before confirming the operation.

#### 8.4.8 Branch Locking

The system shall support branch locking to prevent modifications when a branch should be read-only. Branch locks must:

- Be applicable only to change order branches (main branch cannot be locked)
- Include a reason for locking, recorded with timestamp and user information
- Prevent all modification operations (create, update, delete) on locked branches
- Allow read-only access to locked branch data
- Support unlock operations by authorized users (project managers and administrators)
- Maintain lock history through audit logs

Lock status must be visible in the user interface and API responses, clearly indicating when a branch is locked and why.
...
- **Direct Range Queries**: By utilizing the Composite PK, Time Machine queries are simplified to range queries on the specific branch partition (e.g., `WHERE id=X AND branch=Y AND valid_from <= T < valid_to`).
- **Scenarios Supported**:
...
The system shall provide a merged view service that displays entities from both main and branch branches in a unified view, clearly indicating the change status of each entity:

- **Unchanged**: Entities that exist in both branches with identical data
- **Created**: Entities that exist only in the branch
- **Updated**: Entities modified in the branch, showing the branch version
- **Deleted**: Entities that exist in main but are deleted in the branch

The merged view must support both WBE and CostElement entities, maintaining proper hierarchical relationships and enabling users to understand the complete impact of branch changes before merging.
...
- Branch status indicators (active, locked, merged) shall be visible in the selector
...
The versioning system (as described in section 8.4) provides an integral component of the audit trail by preserving immutable version history for all entities. Each version record captures the complete state of an entity at a point in time, enabling reconstruction of entity evolution and supporting forensic analysis of data changes. For branch-enabled entities, the audit trail includes branch context, allowing tracking of changes within specific change order branches before they are merged into the main branch.
